...
public class Asterisks extends JComponent {
	...
	public void paintComponent(final Graphics g) {	// <2>
		...
		for (int index = 0; index < stringSize; index++) {
			paintAsterisk(g2d, ASTERISK_CYRCLE_RADIUS, index * 180 / stringSize, ASTERISK_SIZE);
		}

		for (int index = 0; index < stringSize; index++) {
			paintLetter(g2d, ASTERISK_CYRCLE_RADIUS, 180 + index * 180 / stringSize, LETTER_SIZE, HELLO_WORLD.charAt(index));
		}
	...
	private void fillBackground(final Graphics2D g2d) {		// <4>
		final RadialGradientPaint	rgp = new RadialGradientPaint(0.0f, 0.0f, (float)(0.75f*WINDOW_SIZE), new float[]{0.0f, 1.0f}, new Color[]{Color.YELLOW, Color.BLACK});
	...
	private void paintAsterisk(final Graphics2D g2d, final double radius, final double angle, final double scale) { // <5>
	...
	private void paintLetter(final Graphics2D g2d, final double radius, final double angle, final double scale, final char symbol) {	// <6>
		final Color				oldColor = g2d.getColor();
